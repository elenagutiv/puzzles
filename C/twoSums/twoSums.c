...
// One solution would be to use as key some object that combines the information of the integer found
// and the index position (or something else that differentiates same integers at different positions)


// The one-pass-hash solution avoids the latter case as every element is introduced in the hash table only when
